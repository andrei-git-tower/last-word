...
export function TypeformChat({
  ...
  const btnTextColor = primaryColor || "#1a1a2e";
    ...
    return (
      ...
      >
        <div style={cardStyle} className="text-center max-w-xs">
          <p className="text-3xl font-bold text-white mb-3">Quick question</p>
          ...
          <button
            ...
            style={{ backgroundColor: "white", color: btnTextColor }}
          >
            Let's go →
          </button>
        </div>
  ...
  if (complete) {
    return (
      <div
        ...
        style={{ backgroundColor: bgColor, fontFamily }}
      >
        <div className="text-center max-w-xs" style={{ animation: "tf-enter 0.4s cubic-bezier(0.4,0,0.2,1) both" }}>
          ...
          <p className="text-white/60 text-sm">Your feedback means a lot to us.</p>
        </div>
      </div>
    );
  }
    ...
    >
      ...
      <div className="h-0.5 bg-white/10 shrink-0">
        <div
          ...
          style={{ width: `${Math.min(10 + stepCount * 18, 88)}%` }}
        />
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-8 pb-8 min-h-0">
        <div style={cardStyle}>
          ...
          <div className="flex items-center gap-1.5 mb-5">
            <span className="text-xs font-semibold text-white/50 tabular-nums">{stepCount + 1}</span>
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" style={{ color: "rgba(255,255,255,0.5)" }}>
          ...
          {/* Question text */}
          <p className="text-2xl font-bold text-white leading-snug mb-7">
            {currentQuestion}
          </p>
          ...
          <textarea
            ...
            rows={3}
            disabled={loading}
            className="w-full bg-transparent text-white text-lg placeholder:text-white/30 resize-none outline-none pb-2 disabled:opacity-50"
          ...
          <div className="flex items-center gap-3 mt-5">
            <button
              ...
              className="flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-semibold hover:opacity-90 transition-opacity disabled:opacity-40"
              style={{ backgroundColor: "white", color: btnTextColor }}
            >
              {loading ? (
                <>
